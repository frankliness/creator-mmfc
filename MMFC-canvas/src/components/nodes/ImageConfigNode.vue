...
const initialImageModelConfig = getModelConfig(localModel.value)
const localSize = ref(props.data?.size || initialImageModelConfig?.defaultParams?.size || DEFAULT_IMAGE_SIZE)
const localQuality = ref(props.data?.quality || initialImageModelConfig?.defaultParams?.quality || '2K')
...
onMounted(() => {
  ...
  if (!localModel.value || !isModelAvailable) {
    ...
    localQuality.value = modelConfig?.defaultParams?.quality || localQuality.value
    localSize.value = modelConfig?.defaultParams?.size || localSize.value
    updateNode(props.id, {
...
const handleModelSelect = (key) => {
  ...
  if (!defaultSize && newSizeOptions.length > 0) {
    defaultSize = newSizeOptions.find(o => o.key === config?.defaultParams?.size)?.key || newSizeOptions[0].key
  }
...
const handleQualitySelect = (quality) => {
  ...
  if (newSizeOptions.length > 0) {
    const preferredSize = currentModelConfig.value?.defaultParams?.size
    const defaultSize = newSizeOptions.find(o => o.key === preferredSize)?.key || newSizeOptions[0]?.key
