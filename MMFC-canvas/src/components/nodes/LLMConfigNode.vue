...
const handleSelect = (item) => {
  ...
  const defaultData = {
    imageConfig: { model: DEFAULT_IMAGE_MODEL, size: DEFAULT_IMAGE_SIZE, label: '文生图' },
...
onMounted(() => {
  ...
  if (!model.value || !isModelAvailable) {
    ...
    model.value = modelStore.selectedChatModel || availableModels[0]?.key || DEFAULT_CHAT_MODEL
    updateConfig()
  }
...
// 默认模型使用选中的模型
const model = ref(props.data?.model || modelStore.selectedChatModel || DEFAULT_CHAT_MODEL)
// Format options | 格式选项
const formatOptions = [
...
const resolveUserInput = (text) => {
  ...
  for (const mention of mentions) {
    const node = nodes.value.find(n => n.id === mention.nodeId)
    const placeholder = new RegExp(`@\\[${mention.nodeId}(?:\\|[^\\]]+)?\\]`, 'g')
    ...
    if (node?.type === 'image' && node.data?.url) {
      ...
      resolvedText = resolvedText.replace(placeholder, '')
      continue
    }

    if (node?.type === 'text' && node.data?.content) {
      resolvedText = resolvedText.replace(placeholder, node.data.content)
      continue
    }

    if (node?.type === 'llmConfig' && node.data?.outputContent) {
      resolvedText = resolvedText.replace(placeholder, node.data.outputContent)
    }
  }

  return {
    text: normalizeWhitespace(resolvedText),
    images
  }
}
