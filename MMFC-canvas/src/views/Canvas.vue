    ...
    <AppHeader class="bg-[var(--bg-secondary)]">
      <template #left>
        ...
        <BrandLogo
          ...
          :show-text="!isMobile"
          subtitle=""
          title-class="text-sm font-semibold tracking-[0.14em] text-[var(--text-primary)]"
        />
...
import { nodes, edges, addNode, addNodes, addEdge, addEdges, updateNode, initSampleData, loadProject, saveProject, clearCanvas, canvasViewport, updateViewport, undo, redo, canUndo, canRedo, manualSaveHistory, startBatchOperation, endBatchOperation, currentProjectId } from '../stores/canvas'
...
// 集成版：暂未接入 Gemini 视频生成，工具栏与节点菜单不暴露视频入口。
// 待后端实现后改为 true 即可恢复。
const ENABLE_VIDEO_NODES = false
...
// Load project by ID | 根据ID加载项目（已改为异步：projects store 走 HTTP）
...
watch(
  ...
  async (newId, oldId) => {
    if (newId && newId !== oldId) {
      if (oldId) {
        try { await saveProject() } catch (err) { console.warn('[Canvas] saveProject before switch failed:', err) }
      }
...
onMounted(async () => {
  ...
  try {
    ...
    console.warn('[Canvas] initProjectsStore failed:', err)
  }
...
onUnmounted(() => {
  ...
  // 离开前同步保存（fire-and-forget）
  saveProject()?.catch?.((err) => console.warn('[Canvas] saveProject on unmount failed:', err))
})
</script>
