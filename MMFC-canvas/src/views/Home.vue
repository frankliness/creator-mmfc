    ...
    <main class="max-w-5xl mx-auto px-4 py-8 md:py-16">
      ...
      <section class="text-center mb-12">
        <div class="flex items-center justify-center mb-6">
          <BrandLogo
            :icon-size="68"
            subtitle="Google Gemini 官方 API 驱动的多模态无限画布"
            title-class="text-3xl md:text-5xl font-semibold tracking-[0.18em] text-[var(--text-primary)]"
            subtitle-class="text-sm md:text-base text-[var(--text-secondary)]"
          />
        </div>
...
// 集成版：API Key 由平台后端注入，本地视为已配置
const showApiSettings = ref(false)
const isApiConfigured = computed(() => true)

// 刷新 API 配置状态（保留以避免破坏外部引用）
  ...
  switch (key) {
    ...
    case 'duplicate': {
      try {
        ...
        console.error('[Home] duplicateProject failed:', err)
      }
      break
    }
    case 'delete':
      dialog.warning({
        ...
        onPositiveClick: async () => {
          try {
            ...
            console.error('[Home] deleteProject failed:', err)
          }
        }
      })
      break
...
const confirmRename = async () => {
  if (renameTargetId.value && renameValue.value.trim()) {
    try {
      ...
      console.error('[Home] renameProject failed:', err)
    }
  }
...
// 集成版：API Key 由后端管理，前端永远视为已就绪，不再拦截
