...
当前代码基线相比 v1.4.0，已经把多 Provider 体系补齐为一套更完整的运营闭环：

- `ProviderCredential` 不再只是“某 provider 的一条 key”，而是带 `purposes` / `modelKeys` 作用域的共享凭据池；
- `ModelRegistry` 成为各用途模型能力、排序、默认值和 provider 偏好的事实来源；
- `PromptTemplate` + `PromptVersion` 已具备版本历史、回滚、按 provider 生效的能力；
- `CanvasImageTask` 异步任务链路与画布前端恢复轮询逻辑已形成稳定闭环。
