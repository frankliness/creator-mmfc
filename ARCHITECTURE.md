...
2. `web/src/worker/`
   ...
   - 视频任务（Seedance）：15s 节奏，轮询远端任务状态、写回结果、下载视频、上传阿里云 OSS 并入库 `SeriesAsset` + 同步 BytePlus（v2.0.0；旧 GCS 链路保留只写）
   - 画布图任务（v1.4.0）：3s 紧节奏，处理 `CanvasImageTask` 异步生图（gpt-image-1 / 慢 Gemini），落盘 + 写资产 + 写审计
3. `admin/server` + `admin/web`
   管理后台。后端基于 Fastify，前端基于 Vue 3 + Ant Design Vue，负责用户、项目、任务、Prompt、全局配置、审计等运营能力。**v1.8.0 起支持细粒度权限管理系统（14 分栏 × 2 操作）**；**v1.9.0 起新增 Series 管理（5 Tab 详情 / 预算调整 / 成员管理）**；**v2.0.0 起在 Series 详情新增"素材组"Tab，管理 BytePlus Asset Group 绑定**。
4. `MMFC-canvas/`
...
- `web` 面向普通创作者；
- `worker` 面向异步任务执行；
- `admin` 面向运营与系统管理（含权限分配）；
- `MMFC-canvas` 面向可视化 AI 创作体验。

### 1.1 上一版本关注点（v1.8.0）

v1.8.0 完成了管理后台的权限管理系统重构：

- **权限矩阵**：14 个业务分栏 × `read` / `write` 两类操作，细粒度控制管理员访问范围
- **角色体系升级**：SUPER_ADMIN（全权）、ADMIN（可配矩阵）、OPERATOR（受限用户）
- **数据库**：`AdminUser.permissions JSONB?` 存储权限矩阵，`deletedAt DateTime?` 支持软删，禁用 / 软删后旧 token 立即失效
- **前端菜单 / 路由 / 按钮**：根据权限动态渲染，菜单动态过滤，无权限路由自动跳转 403
- **审计完善**：所有管理员操作（创建、更新、删除）写审计日志，密码永不落审计
- **管理页面**：管理员列表新增权限矩阵编辑、禁用、删除、重置密码功能

### 1.2 上一版本关注点（v1.9.0）

v1.9.0 在 v1.8.0 基础上引入用户端项目治理与数据保护两大主线：

**用户端项目治理（Series 体系）**

- **Series → Episode 重定义**：不重命名旧 Project，新增 `Series` 承载产品语义"项目"；`Project` 重定义为 Series 下的"集数（Episode）"
- **三角色 RBAC**：OWNER（导演）/ PRODUCER（制作者）/ VIEWER（只读）；后端每次请求实时查 `ProjectMember`，不缓存
- **资源预算池**：Seedance token 预扣 / 提交 / 释放（事务 + idempotencyKey 防重）；Canvas 成功调用次数硬上限（达 totalBudget 立即 429）
- **OWNER buffer 调配**：buffer ↔ 集数双向调配，全程写 `BudgetEvent` 审计
- **共享 lib**：`series-budget.ts` / `seedance-pricing.ts` / `series-membership.ts` / `workspace-guards.ts`
- 新模型：`Series` / `ProjectMember` / `SeriesResourceBudget` / `ProjectResourceAllocation` / `BudgetEvent`

**数据保护（P0 安全增强）**

- **画布快照乐观锁**：`CanvasProject.version` + 客户端 `baseVersion`；服务端事务内 `FOR UPDATE` 比对，stale → `409 STALE_BASE_VERSION`
- **异常缩量拦截**：新快照 nodes < 旧 nodes × 50% → `409 SNAPSHOT_SHRINK_DETECTED`
- **快照历史**：`CanvasSnapshotHistory` 保留最近 30 版本，覆写前自动入库
- **单会话强制**：`User.activeSessionId`；登录生成新 sid 写 DB，旧 tab 下次请求 sid 不一致即被踢
- **客户端冲突感知**：`MMFC-canvas` autosave 失败立即停写 + 模态框引导刷新；指数退避（300ms / 900ms）应对 5xx

**并发调度升级**

- `ProviderCredential.concurrencyByModel JSON` 字段：按 (渠道, 模型) 独立设并发上限
- Worker 双层并发：(user, model) + (credential, model)，避免单一模型把渠道打满
- 默认 `globalLimit` 调高到 200（仅作安全上限）

**Canvas 集成**

- 新建画布弹窗支持选 Series；`CanvasProject.seriesId` 落库
- 画布生图调用从 Series 全局 Canvas 预算池扣减（`committedUsage += 1`）
- 文字节点拖拽缩放（右下角手柄 + `node.data` 持久化 + 感知 zoom）

### 1.3 本次版本关注点（v2.0.0）

v2.0.0 聚焦"资产链路统一"。在 v1.9.0 的 Series 项目空间基础上，建立 **Series 级统一素材库**，整合 **阿里云 OSS** 作为长期对象存储、**BytePlus Asset Library** 作为 Seedance 视频生成的资产引用源，并集成 v1.10.x 主分支全部改动（displayName / owner 同步 / session-guard / Token 报告 / Canvas stats）。

**素材库统一抽象（v2.0.0 新模型）**

- `SeriesAssetGroup`：Series ↔ BytePlus Asset Group 1:1 软绑定，状态 `ACTIVE` / `FAILED` / `UNBOUND`。Group 创建失败时 Series 仍创建成功，Admin 可重试 / 改绑
- `SeriesAsset`：统一资产表。四种 `source`：`MANUAL_UPLOAD` / `CANVAS_GENERATED` / `VIDEO_RESULT` / `VIDEO_TAIL_FRAME`。五种 `byteplusSyncStatus`：`NOT_SYNCED` / `SYNCING` / `PROCESSING` / `SYNCED` / `FAILED`
- 唯一约束 `(seriesId, normalizedName)`：同 Series 全类型唯一（normalizedName = `trim → lowercase → 多空白折叠`）
- `Storyboard` 加 `generationMode`（FIRST_FRAME / MULTIMODAL）+ `assetRefs`（JSON 结构化引用）；老 `assetBindings/seedanceContentItems` 字段保留只读
- `GenerationTask` 加 `ossVideoKey / ossVideoUrl / lastFrameUrl / lastFrameAssetId / videoAssetId` 五个字段

**对象存储（双链路并存）**

- 新链路：**阿里云 OSS**，所有 v2.0.0 资产
- 老链路：GCS 仅 video-persist 写，**不再写入** SeriesAsset，保留兼容
- OSS object key 命名安全化：`series/{seriesId}/assets/{assetId}.{ext}`（手动上传 / Canvas）或 `series/{seriesId}/episodes/EP{n}/{storyboardId}_{kind}.{ext}`（Worker 产物）。**永不含中文**，展示名走 DB
- 私读 bucket 时给 BytePlus 用预签名 URL（TTL 2h），公读 bucket 也兼容

**BytePlus Asset Library 接入**

- Endpoint：`https://open.byteplusapi.com/`
- 认证：Volcengine V4 HMAC-SHA256（service=`ark`, region=`ap-southeast-1`, version=`2024-01-01`）
- 5 个 action：CreateAssetGroup / GetAssetGroup / ListAssetGroups / CreateAsset / GetAsset
- 同步模式：调用方立即返回 `SYNCING`，前端 3 秒轮询 `refresh-byteplus` 端点拉状态

**分镜资产绑定全面重做**

- 用户端取消 `assetsJson` / `asset_uri` 手填，分镜编辑器走 `asset-picker-modal.tsx`
- FIRST_FRAME 模式：first_frame 必填、last_frame 可选、禁用 reference_*
- MULTIMODAL 模式：reference_image 1-9 / reference_video 0-3（总时长 ≤ 15s）/ reference_audio 0-1，不能只有音频
- 后端 `storyboard-asset-resolver.ts` 在 submit 时把 `assetRefs` 解析为 Seedance content[]（全部 `asset://<byteplusAssetId>`），同步校验 Group ACTIVE、资产 SYNCED、模式互斥、模型能力

**Worker 自动资产化**

- Seedance 视频成功后 Worker 自动下载视频 + 尾帧 → OSS → SeriesAsset(VIDEO_RESULT / VIDEO_TAIL_FRAME) → 异步 BytePlus 同步
- 尾帧策略：Seedance API URL 优先，缺失时 `@ffmpeg-installer/ffmpeg` 抽 mp4 末帧（**捆绑跨平台二进制，不依赖系统 ffmpeg**）
- `seedance.ts.getTaskStatus` 透传 `raw` JSON，便于 Worker 兼容解析尾帧字段名

**Canvas → 素材库同步**

- Canvas `ImageNode.vue` 新增"同步到素材库"按钮 + 命名 modal
- 后端 `/api/workspace/series/:seriesId/assets/from-canvas` 读 CanvasAsset 二进制 → 上 OSS → 建 SeriesAsset(CANVAS_GENERATED)，默认 NOT_SYNCED 待用户手动同步

**Admin 后台**

- 创建 Series 表单加 Asset Group 区：绑定已有 / 新建（事务外异步调 BytePlus，失败不阻塞 Series 创建）
- 详情页加"素材组" Tab，支持重试 / 改绑 / 解绑

**v1.10.x 兼容**

主动合入 main 1.10.0–1.10.3 全部改动：

- v1.10.0：Storyboard.displayName + inline rename + picker 联动 / Series owner 同步事务化 / 更换导演 UI / formatEpisodeTitle helper / 单设备登录 session-guard / 用户名编辑 / Canvas owner 同步
...
- 主要职责：
  - **视频任务**（`GenerationTask` / Seedance）：轮询远端状态、写回 token 消耗、下载视频、可选上传 GCS（legacy）
  - **v2.0.0 自动资产化**（视频任务额外动作）：成功后下载视频 + 尾帧 → 阿里云 OSS → 创建 `SeriesAsset(VIDEO_RESULT / VIDEO_TAIL_FRAME)` → 异步触发 BytePlus 同步；尾帧 API URL 优先，缺失时 `@ffmpeg-installer/ffmpeg` 抽 mp4 末帧（捆绑跨平台二进制，不依赖系统 ffmpeg）
...
**单会话强制（v1.9.0）**：登录时生成新的 `sid = randomBytes(16).hex` 写入 `User.activeSessionId` 与 JWT 的 `token.sid`。后续每次 `jwt` callback 都比对 `token.sid === db.activeSessionId`，不一致 → 返回空 token（前端视为未登录）。效果：同一账号在 B 设备 / B 浏览器登录后，A 设备旧 tab 下一次请求即被踢。

### 5.1bis 用户端项目治理（v1.9.0 新增）

引入 Series 三角色模型，与 legacy Project 作者权限并存：

- **legacy Project**（`Project.seriesId = null`）：仍由 `Project.userId` 单一作者控制（与 v1.8.0 行为一致）
- **Series Project**（集数）：通过 `ProjectMember.role` 决定权限
  - `OWNER`：完全控制 + buffer 调配 + 集数锁定 / 解锁
  - `PRODUCER`：可读可写、可提交分镜消耗预算，但不可改归属或锁定集数
  - `VIEWER`：只读
- 鉴权统一走 `web/src/lib/series-membership.ts` 的 `assertEpisodeAccess(userId, projectId, "read"|"write")`，写操作还会校验 `Project.lockedReason`
...
1. **短剧/分镜/视频生成链路**
   - `Series`（v1.9.0：产品语义"项目"，承载成员与预算）
   - `Project`（v1.9.0 后语义为"集数 / Episode"；legacy 路径下仍是创作单元，`seriesId = null`）
   - `Storyboard`
   - `GenerationTask`
2. **AI Canvas 创作链路**
   - `CanvasProject`（v1.9.0：可选 `seriesId` 绑定 Series；`version` 字段做乐观锁）
   - `CanvasNode`
   - `CanvasEdge`
   - `CanvasAsset`
   - `CanvasAiCall`
   - `CanvasImageTask`
   - `CanvasSnapshotHistory`（v1.9.0：覆写前的旧快照，最多保留 30 版本）

辅助域包括：

- 用户与配置：`User`、`UserApiConfig`、`GlobalConfig`
- v1.9.0 项目治理：`ProjectMember`、`SeriesResourceBudget`、`ProjectResourceAllocation`、`BudgetEvent`
- **v2.0.0 素材库**：`SeriesAssetGroup`（Series ↔ BytePlus Group 软绑定）、`SeriesAsset`（统一资产）
- Prompt 体系：`PromptTemplate`、`PromptVersion`
- 可观测性：`TokenUsageLog`、`UserActionLog`、`AuditLog`

**v2.0.0 资产链路**（新增第 3 条主业务线）：

3. **Series 素材库 + BytePlus 同步链路**
   - `SeriesAssetGroup`：每个 Series 对应一个 BytePlus Asset Group（1:1）
   - `SeriesAsset`：所有资产（手动上传 / Canvas 生成 / 视频结果 / 视频尾帧）统一登记
   - 阿里云 OSS：长期对象存储；object key 用 UUID，永不含中文
   - BytePlus Asset Library：异步同步资产，给 Seedance 提供 `asset://<assetId>` 引用协议
...
- `quota`：JSON 配额配置
- `remark`：后台备注
- `emailVerified`：v1.7.0 注册流程通过验证码后置为 `now()`
- `canSelfCreateProject`（v1.9.0）：是否允许自建 legacy Project。迁移时把已有用户设 `true`，新注册默认 `false`；`GlobalConfig.allow_user_self_create_project` 为全局总开关
- `activeSessionId`（v1.9.0）：当前生效的 NextAuth sid。登录时生成新 sid 写入此字段，旧 tab 因 sid 不一致下次请求即被踢，实现单会话强制
...
### 7.1bis 项目治理（v1.9.0 新增）

#### `Series`

产品语义"项目"。承载成员、资源预算、风格默认值。

关键字段：

- `name / description / status (ACTIVE/LOCKED/ARCHIVED) / ownerId? / createdBy?`
- v1.9.0 末：`defaultStyle / defaultRatio / defaultResolution / defaultSeed`——集数提交时由 Series 默认值统一注入

关联：1:N `Project`（集数）、1:N `ProjectMember`、1:N `SeriesResourceBudget`、1:N `BudgetEvent`、1:N `CanvasProject`。

#### `ProjectMember`

Series 成员关系。

- `seriesId / userId / role (OWNER/PRODUCER/VIEWER) / status / createdBy?`
- `@@unique([seriesId, userId])`
- 后端每次请求实时查，不缓存

#### `SeriesResourceBudget`

按 (Series, provider, modelKey, budgetScope, metricType) 的预算池。

- `metricType: TOKEN | SUCCESS_COUNT`——Seedance 走 TOKEN（预扣 / 提交 / 释放），Canvas 走 SUCCESS_COUNT（成功后锁定）
- `totalBudget / committedUsage / reservedUsage / unallocatedBudget`
- `isHardCap`：达上限是否硬拒（Canvas 默认 true）
- `@@unique([seriesId, provider, modelKey, budgetScope, metricType])`

#### `ProjectResourceAllocation`

可选的 Series 预算 → 集数级分配（OWNER buffer 调配的具体落点）。

#### `BudgetEvent`

预算变更审计：`USAGE_RESERVE / USAGE_COMMIT / USAGE_RELEASE / BUFFER_ALLOCATE / BUFFER_RELEASE / ADMIN_ADJUST / SERIES_BUDGET_CREATE`。记录 `before / after` 数值快照与 `operatorId / operatorRole / reason`。

### 7.1ter 素材库（v2.0.0 新增）

#### `SeriesAssetGroup`

Series 与 BytePlus Asset Group 的 1:1 软绑定。

- `@@unique seriesId`：一个 Series 最多一个 Group
- `provider`：默认 `byteplus`（预留扩展）
- `groupId`：BytePlus 返回的 group id；`status=FAILED` 时为空
- `groupName / groupType / projectName`：BytePlus 端的展示属性
- `status`：`ACTIVE`（已绑定）/ `FAILED`（创建调用失败，可重试）/ `UNBOUND`（手动解绑）
- `error`：失败原因，便于 Admin 重试时定位

#### `SeriesAsset`

统一资产表，承载所有 v2.0.0 新增的资产对象。

- `source`：
  - `MANUAL_UPLOAD`：用户从素材库上传
  - `CANVAS_GENERATED`：Canvas 生图 → 用户手动同步素材库
  - `VIDEO_RESULT`：Worker 视频生成成功后自动创建
  - `VIDEO_TAIL_FRAME`：Worker 抽取视频末帧（API URL 优先 / ffmpeg 抽帧兜底）
- `type`：`IMAGE / VIDEO / AUDIO`
- `name`：用户原始展示名，可含中文
- `normalizedName`：`trim → lowercase → 多空白折叠`
- `@@unique([seriesId, normalizedName])`：同 Series 全类型唯一
- `ossBucket / ossObjectKey / ossPublicUrl`：阿里云 OSS 存储位置（key 安全化，永不含中文）
- `thumbnailUrl?`：可选缩略图（v2.0.0 暂未生成，预留字段）
- `byteplusGroupId / byteplusAssetId / byteplusAssetName / byteplusAssetType`：BytePlus 端对应记录
- `byteplusSyncStatus`：`NOT_SYNCED / SYNCING / PROCESSING / SYNCED / FAILED`
- 反向引用字段：`projectId / storyboardId / generationTaskId / canvasProjectId / canvasAssetId`，便于追溯来源

#### `Storyboard`（v2.0.0 字段扩展）

- `displayName?`：用户自定义显示名（v1.10.0 主分支引入；v2.0.0 picker 选首帧资产时自动填充）
- `generationMode?`：`FIRST_FRAME / MULTIMODAL`，空表示 legacy 老数据
- `assetRefs?`：JSON 结构化资产引用，形如 `{first_frame_asset_id, last_frame_asset_id, reference_image_asset_ids[], reference_video_asset_ids[], reference_audio_asset_id}`
- 老 `assetBindings / seedanceContentItems` 保留只读，新提交不写入

#### `GenerationTask`（v2.0.0 字段扩展）

- `ossVideoKey / ossVideoUrl`：Worker 上 OSS 后回填
- `lastFrameUrl`：Seedance API 返回的尾帧短期 URL（24h 内有效，仅供 Worker 下载用）
- `lastFrameAssetId / videoAssetId`：Worker 自动资产化后关联到 `SeriesAsset.id`

### 7.2 分镜视频业务

#### `Project`

表示一个视频创作项目（v1.9.0 后语义为"集数 / Episode"，但表名不改保兼容）。
...
- `status`：项目生命周期状态
- v1.9.0：`seriesId? / episodeNumber? / episodeTitle? / assigneeId? / lockedReason?`——绑定 Series；锁定后写操作 423
...
- v1.9.0：Series 项目的风格 / 画幅 / 分辨率 / seed 由 `Series.defaultXxx` 注入，集数面板不再渲染这些字段。
...
#### `CanvasSnapshotHistory`（v1.9.0）

覆写前的旧快照备份。每次 `PUT /api/canvas/projects/[id]/snapshot` 成功时，事务内自动把"即将被覆盖"的 nodes/edges/viewport 整体写入此表，最多保留 30 条（按 `createdAt desc` 截断），用于后续人工恢复或调查。

关键字段：

- `projectId / version / nodesJson / edgesJson / viewportJson`
- `nodeCount / edgeCount`（便于检索）
- `userId / userAgent / ipAddress / clientId`（审计来源）
...
#### 画布生图并发调度（v1.6.0 引入，v1.9.0 升级）

`pollCanvasImageTasks.ts` 3 秒紧节奏轮询 `CanvasImageTask`，按多渠道轮转 + 多层并发控制分发任务：

- **全局上限**：`globalLimit`（v1.9.0 默认 200，仅作安全上限）
- **(user, model) 层**：单用户在单模型上的最大 RUNNING 数（防一人独占）
- **(credential, model) 层（v1.9.0 新增）**：单凭据在单模型上的并发上限
  - 优先读 `ProviderCredential.concurrencyByModel JSON[modelKey]`
  - 缺失时 fallback 到旧字段 `ProviderCredential.concurrency`
  - 解决"Azure gpt-image-2 实测并发 6、但 dall-e-3 可以 30"这种异构上限场景
- **渠道选择**：在 scope 命中的凭据中选 RUNNING 最少者，避免单渠道被打满
- **429 冷却**：命中限流 → `ProviderCredential.cooldownUntil`，任务回退 PENDING 由下 tick 其他渠道接手
...
为了避免多 tab / 多设备同时编辑同一画布时的覆盖丢数据，v1.9.0 引入了三层保护：

**1. 乐观锁（version 字段）**

- `CanvasProject.version Int @default(1)`，每次成功 snapshot += 1
- 客户端 `fetchProjectDetail` 时拿到 `version`，存入 `projectVersions Map`
- 每次 `PUT /api/canvas/projects/[id]/snapshot` 携带 `baseVersion`
- 服务端事务内 `SELECT ... FOR UPDATE` 锁行，比对当前 version：不一致 → `409 STALE_BASE_VERSION`
- 命中后客户端把项目加入 `conflictedProjectIds`，停止 autosave；Canvas.vue 弹模态框让用户手动刷新

**2. 异常缩量拦截**

- 新快照 `nodes.length < existingNodes × SHRINK_THRESHOLD_RATIO (0.5)` → `409 SNAPSHOT_SHRINK_DETECTED`
- 客户端可在用户显式确认后带 `confirmShrink: true` 覆盖

**3. 快照历史回滚**

- 覆写前事务内把旧 nodes/edges/viewport 写入 `CanvasSnapshotHistory`
- 按 `createdAt desc` 截断保留最近 30 版本
- 字段含 `clientId / userAgent / ipAddress`，便于事后审计

客户端层面：`MMFC-canvas/src/stores/projects.js` 在 5xx / 网络中断时做指数退避（300ms → 900ms）重试；命中 409 立即停 autosave 并 dispatch `canvas:save-conflict` 事件，Canvas.vue 监听弹窗。
